...
1. 调用 `build_qq_container()` 构建 QQ 容器
...
`bootstrap_web.py` 负责装配 Web 入口所需对象（与 QQ 容器解耦）：
...
其中 `WebNoopTransport` 只提供最小能力，Web 场景下回复主要通过 HTTP 接口返回，不依赖 QQ 的主动消息推送链路。
...
- QQ：由 transport 主动发送回复
- Web：通过 HTTP `POST /chat` 响应体返回 `reply`
